...
```
## 移动端适配
项目集成了移动端适配插件，开发时只需按照`375px`宽度的设计稿来写`px`尺寸即可，实际运行会自动被计算成`vw`单位。<br/>
## axios
...
ESLint 是一个ECMAScript/JavaScript 语法规则和代码风格的检查工具，它的目标是保证代码的一致性和避免错误。[ESlint 中文官方网站](http://eslint.cn/)中列出了详细的配置规则，此框架eslint规则的配置在`package.json`文件中。
...
项目支持全局less变量的定义，定义变量的文件为：`src/assets/style/variable.less`。